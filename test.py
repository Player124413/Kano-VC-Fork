...
def GradioSetup():
    ...
    with gr.Blocks(theme='NoCrypt/miku', title="Kano-VC-Fork") as app:
        ...
        with gr.Tabs():
            ...
            with gr.TabItem(i18n("Theme")):
             theme_name = gr.Textbox(
              label=i18n("Enter the theme name:"),
               value="",
    )
    update_button = gr.Button(label=i18n("Update Theme"))

@gr.Interface
def update_theme(theme_name):
    # Проверьте, существует ли тема на серверах Gradio
    if theme_exists(theme_name):
        # Если тема существует, примените ее
        gr.Interface.set_theme(theme_name)
        return i18n("Theme updated successfully!")
    else:
        # Если темы не существует, верните сообщение об ошибке
        return i18n("The entered theme does not exist. Please try again.")
